...
- **Stack after #164, #166 and #167:** `contract` merged as PR #166 (squash 53fc8d2); `scope` merged as PR #167 (squash 73fdd0e, 2026-09-15 ~04:58Z, nine Copilot rounds: folders only from `context/` paths, `--also` branches listed among branches not roots, expand refusals carry the scoped map, README synopsis, corrupt index refuses a grant, `--also` with nothing to widen told what to give, scope-missing logged with ids and tokens, one concrete command in the MCP refusal, touched branches in one pass). `views` → PR #168 (slice 4, #160, opened 2026-09-15 ~05:00Z, Copilot requested, body `scratchpad/prbodies/slice4.md`, 334 tests, screenshots impossible and said so) → `proof` (slice 5, #161, `slice5.md`) → `semantic` (slice 6, #165, `slice6.md`, 337 tests); each rebased with `git rebase --onto <new-base> <old-base> <branch>` after every push below it. Semantic rerun done and committed on `semantic` (`eval/results/2026-09-15-semantic-*`): branch reached 4/4 small, 6/6 large on semantic questions; accuracy 50%/67% semantic vs 100%/83% lexical; medium generates no orientation question; CHANGELOG, RETRIEVAL and ORIENTATION carry the numbers. Owner's report artifact https://claude.ai/artifact/L3v6gNmEh1h9h9enQ3Z2Vy still to be republished with the semantic numbers and the PR links after the stack merges.
