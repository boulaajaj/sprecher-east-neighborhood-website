...
- **Course correction clarified 2026-09-15 (two owner messages):** agents are things in the world, not a concern of the brain; no executor interface; resolution by meaning (smallest meaningful unit, files and lines are evidence); Amibou is the context and routing layer for prompts, semantic navigation primary and the router assisting; the brain never built once; the human's seven questions; access control not expanded. Done: `docs/ORIENTATION.md` rewritten on branch `contract` (4afbb1f, pushed; PR after #164); slice 3 trimmed (work order/result schemas and `src/work.mjs` gone, `src/context.mjs` holds `contextPackage`, worker→client everywhere; #159 rewritten); slice 6 opened as #165 (branch `semantic` off `proof`: `AREA_PARAPHRASE`, orientation `mode` lexical/semantic, harness rows per mode, skill and tool description say the map is the primary path); the stack is contract → scope → views → proof → semantic, all pushed, 337 tests at the top. Epic comment with the gap summary posted on #155. CI and Copilot are blocked on the account: "recent GitHub Actions payments have failed or your spending limit needs to be increased" (commented on #164); nothing merges until the owner fixes it.
- **Slice 2 in review:** PR #164 (issue #158) from `branches`, rebased onto main after #163; Copilot round 1 fixed in 6fe983f (`meaning:` hits distinct from `field:`, cycle-safe `branchMembers`, echo runner chooses once); CI run 34911449415 failed at startup with no runner and was re-run once. Owner's report published as an artifact: https://claude.ai/artifact/L3v6gNmEh1h9h9enQ3Z2Vy (republish after every merge). Stacked locally: `scope` (slice 3, 6081a42+), `views` (slice 4, `mapOf`, `renderMap`, `mapGraphOf`, area categories capped at 12 with `elsewhere`, `/focus?id=*` whole brain), `proof` (slice 5, `tests/compat.test.mjs`, acceptance steps 11 to 14, contract rows shipped); 338 tests pass at the top. Screenshots at six viewports could not be taken: headless Chromium in this container cannot reach the local server and one attempt restarted the container; say so in the slice 4 PR.
