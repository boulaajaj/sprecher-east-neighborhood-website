...
- **Stack after #164 and #166:** `contract` merged as PR #166 (squash 53fc8d2, 2026-09-15 ~03:42Z, five Copilot rounds, all wording: British/American spelling made consistent with the repo's American majority, "progressive resolution" as the page's term). `scope` → PR #167 (slice 3, #159, open ~03:45Z, Copilot requested; body rewritten without the executor interface: `src/context.mjs`, `contextPackage`, 332 tests) → `views` (slice 4, #160, body `scratchpad/prbodies/slice4.md`, 334 tests, say screenshots impossible) → `proof` (slice 5, #161, `slice5.md`) → `semantic` (slice 6, #165, `slice6.md`, 337 tests); each rebased with `git rebase --onto <new-base> <old-base> <branch>` after every push below it. Semantic rerun done and committed on `semantic` (`eval/results/2026-09-15-semantic-*`): branch reached 4/4 small, 6/6 large on semantic questions; accuracy 50%/67% semantic vs 100%/83% lexical; medium generates no orientation question (no area holds a project with a unique state); CHANGELOG, RETRIEVAL and ORIENTATION carry the numbers. Owner's report artifact https://claude.ai/artifact/L3v6gNmEh1h9h9enQ3Z2Vy still to be republished with the semantic numbers and the PR links after the stack merges.
