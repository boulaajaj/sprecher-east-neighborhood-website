...
- **Slice 4 in review:** PR #152 (issue #151) from branch `retrieval`: `LEVEL1_SERVED` (25 lines of the nodes the question touched, `levels.level1Note`), `SOURCE_CAP` at Level 4, `coverage` on candidates so a project outranks the decision or component whose id carries its name while two projects named alike still tie; `eval/run.mjs` with four arms (native search = top three files by question words), counted tokens beside bytes/4, hallucination rate, candidates, `--repeat`, `--size` from `eval/brains.mjs` (8, 72, 970 nodes), `--window` recording overflow. Real runs committed (`eval/results/2026-09-14-*`, two repeats): fixture router 90% at 36% of whole-brain tokens (target met), medium 100% at 10% (met), large whole brain does not fit 180k and router 100% at ~1,700 tokens (met), small tie at 51% (not met, said so). Weekly review now reads `amibou cards --json`, not Level 1. Check-in trig_016vfqDRuQNrMrnv764gabDP at 21:24Z.
