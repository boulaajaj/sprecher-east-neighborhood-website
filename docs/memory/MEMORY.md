...
- **#176 in review:** PR #181 from branch `recall-impl` (worktree `v2recall2`): `src/recall.mjs` (`judge`: named, or at least two distinct words with no tie; a field or meaning word weighs two and is still one word; 600-token ceiling, three nodes), `amibou recall --stdin` reads the hook JSON (`prompt|user_input|user_input_raw`, `cwd`), `brain_recall` tool, bridge installs `HOOK_ENTRY` into `.claude/settings.json` and `.codex/hooks.json` (`mergeHooks` compares canonical JSON, `isOurHook` matches the exact command, `--hooks none`), `bridge.json` lists `hooks`. Eight Copilot rounds so far, each "Findings: None" plus one "previously missed" nit; round 9 pending on the `cli()` helper taking stdin. CI gotcha again: one push (b731e05) did not update the PR head; a later push resynced it; `ci.yml` was dispatched by hand meanwhile.
- **#177 built, PR pending #181's merge:** branch `conversation` (worktree `v2conv`, main and recall-impl merged in): `src/connections/conversation.mjs` (source `conversation`, medium `chat`, record `exchange`), `src/conversation.mjs` (`checkConversation` refuses transcripts, messages, quotes, credentials, codes, card numbers, the sensitive categories; `importConversation` resolves each observation through the identity order; a field a source records without a status stands alone so approving it later never sets a newer status back; evidence the node holds is skipped and recorded), `amibou observe conversation <brain> <file> | --schema | --guide`, `brain_observe` (write grant; refusals `read-only`, `invalid`, `observation`), skill Observe paragraph (BODY_WORDS 380), tools now `[brain_query, brain_recall, brain_observe, brain_expand]`. The vocabulary test bans `conversations` (plural, a provider's records), not the framework's singular source.
- **#178 built, PR pending #177:** branch `seamless` (worktree `v2seam`, on conversation): `tests/seamless.test.mjs` runs the eight scenarios through the hook, the tools and the commands on a synthetic travel brain with the two invariants (context/ untouched before approve; no client named under context/, sources/, proposals); the vertical slice has steps 15 (recall unasked) and 16 (one conversation fact proposed, approved, re-imported to no change); `tests/acceptance.test.mjs` expects sixteen steps.
