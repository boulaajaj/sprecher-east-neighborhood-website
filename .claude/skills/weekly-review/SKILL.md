...
2. **Open work, every domain.** Call `brain_query` with "what is open or in progress across all my projects". Level 1 comes back with every node's status and date; from it, pick every node whose status is not done, closed or decided. Do not stop at the matched nodes.
3. **Each open node** at Level 2: `brain_expand <id>@2`. Note the status, the next action, any open question, and the `updated` date. Go to `@3` only where a node names a dependency you cannot place.
